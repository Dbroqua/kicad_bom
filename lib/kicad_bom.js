...
class kicad_bom {
    ...
    constructor(bomfile, csvfile) {
        ...
        if (csvfile !== undefined) {
            this.csvfile = path.join(path.resolve(process.cwd()), csvfile);
        }
        this.components = [];
        this.grouped = {};
    }
    ...
    exec() {
        ...
        if (that.csvfile === undefined) {
            let cvsBasedOnBomFile = this.bomfile.split('/');
            cvsBasedOnBomFile = cvsBasedOnBomFile[cvsBasedOnBomFile.length -1].replace('.xml','.csv');
            that.csvfile = path.join(path.resolve(process.cwd()), cvsBasedOnBomFile);
        }
